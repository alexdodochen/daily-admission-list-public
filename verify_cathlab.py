...
資料來源：子表格（主治醫師病人清單，8-col canonical），欄位 A=姓名 B=病歷號 C=EMR D=EMR摘要 E=手動序 F=術前診斷 G=預計心導管 H=註記。
...
def get_cathlab_date_for_patient(admission_date_str, doctor, note):
    ...
    - 張獻元週三入院 → 同日 PM（張獻元週三 AM C2 / PM C2 時段；週四無時段不走 N+1）
    - 其他 → N+1
    """
    ...
    if wd == 4:  # Friday
        ...
        cath = dt  # Wednesday same-day (張獻元 週三 AM C2 + PM C2)
...
def read_ordering(sheet_name):
    ...
    for row in data:
        ...
        # Patient row (8-col): A=name B=chart C=EMR D=EMR摘要 E=手動序 F=diag G=cath H=note
