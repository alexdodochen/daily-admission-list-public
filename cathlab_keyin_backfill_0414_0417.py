"""
Backfill cathlab key-in for missing patients on 4/14, 4/15, 4/17.
Same session handles all three cathlab dates.
"""
import time
import json
...
DIAG_IDS = {
    ...
    "pAf": "PDI20090908120040",
    "PSVT": "PDI20090908120033",
    "Sinus nodal dysfunction": "PDI20090908120038",
    "STEMI": "PDI20090908120010",
}
PROC_IDS = {
...
# Each patient: cathlab_date in YYYY/MM/DD format
PATIENTS = [
    # --- 4/14 cathlab (Tue) ---
    {"cathlab_date": "2026/04/14", "name": "梁淳斌", "chart": "14908231",
     "doctor": "劉嚴文", "second": "葉建寬", "room": "H1", "time": "0650",
     "diagnosis": "CAD", "procedure": "Left heart cath."},
    {"cathlab_date": "2026/04/14", "name": "李亨利", "chart": "06258690",
     "doctor": "陳昭佑", "second": "李柏增", "room": "H2", "time": "0650",
     "diagnosis": "pAf", "procedure": "RF ablation"},
    {"cathlab_date": "2026/04/14", "name": "曾敏", "chart": "11985392",
     "doctor": "鄭朝允", "second": None, "room": "C2", "time": "0650",
     "diagnosis": "Sinus nodal dysfunction", "procedure": "PPM"},
    # --- 4/15 cathlab (Wed) ---
    {"cathlab_date": "2026/04/15", "name": "謝好", "chart": "01884136",
     "doctor": "陳儒逸", "second": "葉建寬", "room": "C1", "time": "0602",
     "diagnosis": "STEMI", "procedure": "PCI"},
    {"cathlab_date": "2026/04/15", "name": "李宥蓁", "chart": "21285839",
     "doctor": "廖瑀", "second": None, "room": "C1", "time": "1800",
     "diagnosis": "PSVT", "procedure": "RF ablation"},
    # --- 4/17 cathlab (Fri) ---
    {"cathlab_date": "2026/04/17", "name": "林根田", "chart": "06815735",
     "doctor": "詹世鴻", "second": None, "room": "C2", "time": "0600",
     "diagnosis": "CAD", "procedure": "Left heart cath.",
     "note": "4/7無床改期"},
    {"cathlab_date": "2026/04/17", "name": "陳云英", "chart": "11458318",
     "doctor": "詹世鴻", "second": None, "room": "C2", "time": "0601",
     "diagnosis": "CAD", "procedure": "Left heart cath."},
    # 星期五入院排同日:李文煌 Fri PM H1
    {"cathlab_date": "2026/04/17", "name": "陳進旺", "chart": "13023948",
     "doctor": "李文煌", "second": None, "room": "H1", "time": "1800",
     "diagnosis": "CAD", "procedure": "PCI",
     "note": "Staged PCI for LAD ISR"},
]
...
def add_patient(page, patient, existing_charts):
    chart = patient["chart"]
    cdate = patient["cathlab_date"]
    if chart in existing_charts:
        log(f"  SKIP (exists): {patient['name']} ({chart}) on {cdate}")
        return "skip"
    ...
    log(f"  ADD: {patient['name']} ({chart}) {cdate} {patient['room']} {patient['time']} {patient['doctor']}")
    ...
    try:
        ...
        page.evaluate(f'() => {{ document.querySelector(\'input[name="inspectiondate"]\').value = "{cdate}"; }}')
        ...
        log(f"    ERROR: {e}")
        page.screenshot(path=f"cathlab_backfill_error_{chart}.png")
        return "error"
...
def main():
    with sync_playwright() as p:
        ...
        log(f"=== Backfill Cathlab Key-in: {len(PATIENTS)} patients ===")
        login(page)

        # Phase 1: ADD — query per patient to get latest existing
        log("\n--- Phase 1: ADD ---")
        ...
        by_date = {}
        for pt in PATIENTS:
            by_date.setdefault(pt["cathlab_date"], []).append(pt)
        for d, pts in by_date.items():
            set_date_and_query(page, d)
            final = get_existing_charts(page)
            log(f"[{d}] total in WEBCVIS: {len(final)}")
            for pt in pts:
        ...
        with open("_cathlab_backfill_log.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(LOG))
