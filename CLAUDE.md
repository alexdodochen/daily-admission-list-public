...
- `fetch_emr.py` — Playwright-based EMR fetcher. Session URL + (chart, doctor) pairs → `emr_data_<date>.json`. Sentinel-stamping for race safety. Falls back to whitelist doctors when target has no clinic visit. `NAME_ALIASES` dict normalises doctor-name CJK variants (e.g. 林佳凌↔林佳淩) before link-text match — append new variants here when a doctor's EMR fetch silently fails. See `memory/feedback_doctor_name_variant_lin_jialing.md`.
