...
---
新機器開工前，**先 `ls local_config.py`**。沒有就從別台機器 `scp` 過來，或從 `memory/_private_setup.md`（gitignored，含私有 SHEET_ID）重建：

```python
# local_config.py — gitignored
SHEET_ID = '<private SHEET_ID — see memory/_private_setup.md>'
```

`gsheet_utils.py` 找不到 `local_config.py` → fallback 到預設 public mirror demo sheet。寫入時 silent succeed，使用者打開私有 sheet 會說「沒看到我做的更動」，再去發現是寫錯地方。
...
- 沒有就從 `memory/_private_setup.md` 拿私有 SHEET_ID 寫進去
- 確認 `from gsheet_utils import SHEET_ID; print(SHEET_ID)` 開頭是私有那串（非 `1u2FZE6...`）才開始寫入
